"""
image_2s must be in ./Kitti/testing/image_2/ and camera matricies in ./Kitti/testing/calib/

Uses YOLO to obtain 2D box, PyTorch to get 3D box, plots both

SPACE bar for next image_2, any other key to exit
"""
...
parser.add_argument("--image_2-dir", default="eval/image_2/",
                    help="Relative path to the directory containing image_2s to detect. Default \
                    is eval/image_2/")
...
parser.add_argument("--video", action="store_true",
                    ...
                    By default, this will pull image_2s from ./eval/video")

parser.add_argument("--show-yolo", action="store_true",
                    help="Show the 2D BoundingBox detecions on a separate image_2")
...
def main():
    ...
    if len(model_lst) == 0:
        ...
        # model.load_state_dict(torch.load(weights_path + '/%s'%model_lst[-1], map_location=device))
    ...
    image_2_dir = FLAGS.image_2_dir
    cal_dir = FLAGS.cal_dir
    if FLAGS.video:
        if FLAGS.image_2_dir == "eval/image_2/" and FLAGS.cal_dir == "camera_cal/":
            image_2_dir = "eval/video/2011_09_26/image_2/"
            cal_dir = "eval/video/2011_09_26/"


    img_path = os.path.abspath(os.path.dirname(__file__)) + "/" + image_2_dir
    print("image_2 path: ", img_path)
    ...
    try:
        ...
        print("\nError: no image_2s in %s"%img_path)
        exit()
    ...
    for img_id in ids:
        ...
        print("yolo image_2: ", yolo_img.shape)
        ...
        if FLAGS.show_yolo:
            ...
            cv2.imshow('SPACE for next image_2, any other key to exit', numpy_vertical)
        else:
